...
from puppies import Base, Shelter, Puppy, Profile, Adoptor, association_table
...
for i,x in enumerate(male_names):
	shelt = randint(1,5)
	new_puppy = Puppy(name = x, gender = "male", dateOfBirth = CreateRandomAge(),picture=random.choice(puppy_images) ,shelter_id=shelt, weight= CreateRandomWeight())
	session.add(new_puppy)
	s = session.query(Shelter).filter_by(id = shelt).first()
	s.current_occupancy += 1
	session.add(s)
	print s
	session.commit()
...
for i,x in enumerate(female_names):
	shelt = randint(1,5)
	new_puppy = Puppy(name = x, gender = "female", dateOfBirth = CreateRandomAge(),picture=random.choice(puppy_images),shelter_id=shelt, weight= CreateRandomWeight())
	session.add(new_puppy)
	s = session.query(Shelter).filter_by(id = shelt).first()
	s.current_occupancy += 1
	session.add(s)
...
new_prof = Profile(url = "", special_needs="squeaky chew toy", description="cute", puppy_id=55)
session.add(new_prof)
session.commit()

adopt = Adoptor(name = "Jimmy")
session.add(adopt)
session.commit()
